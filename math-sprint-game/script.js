...
// Code wont update the DOM, spend some time one it to figure it out.
...
bestScoreArray.forEach((bestScore, index) => {
  ...
  if (questionAmount == bestScore.questions) {
    // return best score as a number with a decimal
    const savedBestScore = Number(bestScoreArray[index].bestScore);
    // update if new final score is less or replace zero 
...
function checkTime() {
  if (playerGuessArray.length == questionAmount) {
    clearInterval(timer);
  //  check for wrong ansers and then add penalty time
...
// scroll, store user selections in player guess array
